...
<body>
  <div id="container">
    ...
    <div class="row" id="main">
      ...
      <div id="left">
        <form class="form-horizontal article" role="form" action="../operateTask?operate=publish" onsubmit="return checkInput();">   
          <textarea class="taskInput" name="text" id="text"></textarea>
          <div class="form-group">
            <button type="submit" class="btn btn-default release">发布</button>
          </div>
        </form>
        
        <ul>
        	<%
            		List<Task> tasks = (List<Task>)Dao.select("where task_author = "+user.user_id+" AND tree_level = 1 ORDER BY task_date DESC", Task.class);
			    	for(Task task:tasks){
			    		...
			    		if(task.getParent_task_id()!=null){  //即为转发贴
			    			...
			    			task_text=task_text+"转发自:"+u1.getUser_name()+":"+t.getTask_text();
			    		}
            ...
            <div class="task">
              <div class="userinfo">
                ...
                <div class="userName">
                  <h4><%=user_name %></h4>
                  <!-- 按钮触发模态框 -->
                  <button class="btn btn-primary btn-lg delete" value="<%=task_id %>">&nbsp;&nbsp;删除
                  </button>
                </div>
...
$(".heart").click(function(e){
   ...
   $(".delete").click(function(e){
	   $.post("../operateTask?operate=delete",{task_id:$(e.target).attr('value')},
	              function(data){
